...
```{r}
mod2 <- glmer(correct ~ group + difficulty + (difficulty|subject), family = binomial("logit"), data = exp1_met_trials)
summary(mod2)
```
...
```{r}
mod3 <- lmer(pep ~ group + difficulty + block  + (block|subject), met_pep_df)
summary(mod3)
anova(mod3)
```
...
eta_squared(lm(cbind(PreMood, PreTired, SBPBase, DBPBase, MAPBase, HRbase, PEPBase) ~ group, data = exp1_data))
```
